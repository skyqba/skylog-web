      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:780, margin:'0 auto' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:'1rem', marginBottom:'1.25rem' }}>
          <GlassCard style={{ padding:'1.5rem' }}>
            ...
            {monthData.length > 1 && <div style={{ marginTop:'1rem', height:50 }}><ResponsiveContainer width="100%" height="100%"><LineChart data={monthData}><XAxis dataKey="month" hide /><Line type="monotone" dataKey="count" stroke="#8B5CF6" strokeWidth={2} dot={false} /><Tooltip contentStyle={{ background:'rgba(15,23,42,0.95)', border:'1px solid rgba(139,92,246,0.3)', borderRadius:10, fontSize:'0.78rem', color:'#fff', fontFamily:'var(--font)' }} itemStyle={{ color:'#A78BFA' }} labelFormatter={(l, payload) => { const m = ['Styczeń','Luty','Marzec','Kwiecień','Maj','Czerwiec','Lipiec','Sierpień','Wrzesień','Październik','Listopad','Grudzień']; const yr = payload && payload[0] ? payload[0].payload.year : ''; return (m[parseInt(l)-1] || l) + ' ' + yr }} formatter={(v) => [v + ' skoków', '']} cursor={{ stroke:'rgba(139,92,246,0.3)', strokeWidth:1 }} /></LineChart></ResponsiveContainer></div>}
          </GlassCard>
