...
import { ResponsiveContainer, LineChart, Line, XAxis, BarChart, Bar, PieChart, Pie, Cell, Tooltip } from 'recharts'
...
export default function ProJournal({ jumps, loading, onDelete, onRepeat, repeating, docs = [], urgentDocs = [] }) {
  ...
  const monthData = Object.entries(perMonth).filter(([,c]) => c > 0).sort().slice(-6).map(([m,c]) => ({ month:m.slice(5), year:m.slice(0,4), count:c }))
      ...
      <div style={{ position:'relative', zIndex:1, maxWidth:780, margin:'0 auto' }}>
          ...
          <div style={{ background:'rgba(30,41,59,0.6)', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', border:'1px solid rgba(255,255,255,0.1)', borderTop:'1px solid rgba(255,255,255,0.18)', borderRadius:20, marginBottom:'1rem', overflow:'hidden', boxShadow:'0 8px 32px rgba(0,0,0,0.4)' }}>
            <button onClick={() => setShowDocs(d => !d)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.85rem 1.1rem', background:'transparent', border:'none', cursor:'pointer', color:'#F1F5F9', fontFamily:'var(--font)' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'0.75rem' }}>
                ...
                <span style={{ fontSize:'0.88rem', fontWeight:500, fontFamily:"'Plus Jakarta Sans', Inter, sans-serif" }}>Moje dokumenty</span>
                {urgentDocs.length > 0 && (
                  ...
                  return (
                    <div key={doc.label} style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                      <div style={{ display:'flex', alignItems:'center', gap:'0.6rem' }}>
                        ...
                        <span style={{ fontSize:'0.85rem', color:'#94A3B8', fontFamily:"'Plus Jakarta Sans', Inter, sans-serif" }}>{doc.label}</span>
                      </div>
                      <span style={{ fontFamily:"'Plus Jakarta Sans', Inter, sans-serif", fontSize:'0.78rem', color: doc.noExpiry ? '#10B981' : color }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(280px, 1fr))', gap:'1rem', marginBottom:'1.25rem' }}>
          <GlassCard style={{ padding:'1.5rem' }}>
            ...
            {monthData.length > 1 && <div style={{ marginTop:'1rem', height:50, outline:'none' }}><ResponsiveContainer width="100%" height="100%"><LineChart data={monthData}><XAxis dataKey="month" hide /><Line type="monotone" dataKey="count" stroke="#8B5CF6" strokeWidth={2} dot={false} /><Tooltip contentStyle={{ background:'rgba(15,23,42,0.95)', border:'1px solid rgba(139,92,246,0.3)', borderRadius:10, fontSize:'0.78rem', color:'#fff', fontFamily:'var(--font)' }} itemStyle={{ color:'#A78BFA' }} labelFormatter={(l, payload) => { const m = ['Styczeń','Luty','Marzec','Kwiecień','Maj','Czerwiec','Lipiec','Sierpień','Wrzesień','Październik','Listopad','Grudzień']; const yr = payload && payload[0] ? payload[0].payload.year : ''; return (m[parseInt(l)-1] || l) + ' ' + yr }} formatter={(v) => [v + ' skoków', '']} cursor={{ stroke:'rgba(139,92,246,0.3)', strokeWidth:1 }} /></LineChart></ResponsiveContainer></div>}
          </GlassCard>
