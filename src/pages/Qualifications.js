...
export default function Qualifications() {
  ...
  const alerts = [
cat > "/Users/kuba/Desktop/Sky JumpLog/src/pages/Qualifications.js" << 'ENDOFFILE'
import { useEffect, useState } from 'react'
import { useNavigate } from 'react-router-dom'
import { supabase } from '../supabase'
import Navbar from '../components/Navbar'
            ...
            {hasTandem && (
              <div className="form-group" style={{ marginBottom:0, marginTop:'0.5rem' }}>
                ...
                {daysUntil(tandemExpiry) !== null && (
                  <div style={{ marginTop:'0.5rem', fontSize:'0.82rem', fontWeight:600, color: daysUntil(tandemExpiry) < 0 ? 'var(--danger)' : daysUntil(tandemExpiry) <= 30 ? '#FBBF24' : 'var(--success)' }}>
                    {daysUntil(tandemExpiry) < 0 ? `Wygasło ${Math.abs(daysUntil(tandemExpiry))} dni temu` : daysUntil(tandemExpiry) <= 30 ? `Wygasa za ${daysUntil(tandemExpiry)} dni` : `Ważne jeszcze ${daysUntil(tandemExpiry)} dni`}
                  </div>
                )}
              </div>
            )}
                  ...
                  {insSl && (
                    <div className="form-group" style={{ marginBottom:0 }}>
                      ...
                      {daysUntil(insSlExpiry) !== null && (
                        <div style={{ marginTop:'0.4rem', fontSize:'0.82rem', fontWeight:600, color: daysUntil(insSlExpiry) < 0 ? 'var(--danger)' : daysUntil(insSlExpiry) <= 30 ? '#FBBF24' : 'var(--success)' }}>
                  ...
                  {insAff && (
                    <div className="form-group" style={{ marginBottom:0 }}>
                      ...
                      {daysUntil(insAffExpiry) !== null && (
                        <div style={{ marginTop:'0.4rem', fontSize:'0.82rem', fontWeight:600, color: daysUntil(insAffExpiry) < 0 ? 'var(--danger)' : daysUntil(insAffExpiry) <= 30 ? '#FBBF24' : 'var(--success)' }}>
                  ...
                  {insT && (
                    <div className="form-group" style={{ marginBottom:0 }}>
                      ...
                      {daysUntil(insTExpiry) !== null && (
                        <div style={{ marginTop:'0.4rem', fontSize:'0.82rem', fontWeight:600, color: daysUntil(insTExpiry) < 0 ? 'var(--danger)' : daysUntil(insTExpiry) <= 30 ? '#FBBF24' : 'var(--success)' }}>
