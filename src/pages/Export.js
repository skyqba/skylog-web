...
export default function Export() {
  ...
  const inputStyle = {
    ...
    color: 'var(--text)', fontFamily: 'var(--font)', fontSize: '0.82rem',
  ...
  const toggleAll = () => {
    ...
    if (allSelected) {
      setSelected(prev => { const next = new Set(prev); filteredJumps.forEach(j => next.delete(j.id)); return next })
    } else {
      setSelected(prev => { const next = new Set(prev); filteredJumps.forEach(j => next.add(j.id)); return next })
    }
  }
  ...
  const generatePDF = async () => {
    ...
    if (profile) {
      ...
      if (name) { doc.text(plChar(`Skoczek: ${name}`), 14, headerY); headerY += 6 }
    }
    doc.text(plChar(`Liczba skokow: ${selectedJumps.length}`), 14, headerY)
    doc.text(plChar(`Data wydruku: ${new Date().toLocaleDateString('pl-PL')}`), 200, 18)
    autoTable(doc, {
      startY: headerY + 8,
      head: [[t('export.col_lp'), t('export.col_number'), t('export.col_date'), t('export.col_city'), t('export.col_parachute'), t('export.col_altitude'), t('export.col_delay'), t('export.col_aircraft'), t('export.col_type'), t('export.col_notes')]],
      body: selectedJumps.map((j, index) => [
        index + 1, j.number, fmt(j.jump_date),
        plChar(j.city) || '', plChar(j.parachute) || '',
        j.altitude || '', j.delay || '',
        plChar(j.aircraft) || '', plChar(j.jump_type) || '', plChar(j.notes) || '',
      ]),
  ...
  const printJumps = () => {
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>JumpLogX</title>
      <style>body{font-family:Arial,sans-serif;font-size:11px;margin:20px;color:#111}h1{font-size:18px;margin-bottom:4px}.meta{font-size:11px;color:#555;margin-bottom:16px}table{width:100%;border-collapse:collapse}th{background:#6C63FF;color:white;padding:6px 8px;text-align:left;font-size:10px}td{padding:5px 8px;border-bottom:1px solid #eee;font-size:10px}tr:nth-child(even){background:#f8f8fc}@media print{body{margin:10px}}</style>
      </head><body>
      <h1>JumpLogX</h1>
      <div class="meta">${name ? `${name} &nbsp;|&nbsp; ` : ''}${selectedJumps.length} &nbsp;|&nbsp; ${new Date().toLocaleDateString('pl-PL')}</div>
      <table><thead><tr><th>Lp.</th><th>Nr</th><th>Data</th><th>Miasto</th><th>Spadochron</th><th>Wys.(m)</th><th>Opóz.(s)</th><th>Samolot</th><th>Typ</th><th>Uwagi</th></tr></thead>
  ...
  const thStyle = (col) => ({
    padding: '0.65rem 0.75rem', textAlign: 'left', fontFamily: 'var(--mono)',
    fontSize: '0.65rem', color: sortBy === col ? 'var(--accent2)' : 'var(--muted)',
    ...
    <div>
      ...
      <div style={{ maxWidth: 980, margin: '0 auto', padding: '1.5rem 1rem' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', marginBottom:'1.5rem' }}>
          ...
          <h2 style={{ fontFamily:'var(--head)', fontSize:'1.3rem', fontWeight:800 }}>{t('export.title')}</h2>
        </div>
            ...
            <div style={{ background:'var(--bg2)', border:'1px solid var(--border2)', borderRadius:'var(--r2)', marginBottom:'1rem', overflow:'hidden' }}>
              <button onClick={() => setShowFilters(f => !f)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0.85rem 1.25rem', background:'transparent', border:'none', cursor:'pointer', color:'var(--text)', fontFamily:'var(--font)' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'0.75rem' }}>
                  ...
                  <span style={{ fontSize:'0.88rem', fontWeight:500 }}>{t('export.filter_title')}</span>
                  {activeFiltersCount > 0 && (
                    <span style={{ background:'rgba(108,99,255,0.2)', border:'1px solid rgba(108,99,255,0.4)', borderRadius:20, padding:'0.1rem 0.55rem', fontSize:'0.72rem', color:'var(--accent2)', fontWeight:600 }}>
                      {t('export.active_filters', { count: activeFiltersCount })}
                    </span>
                  )}
                </div>
              ...
              {showFilters && (
                <div style={{ borderTop:'1px solid var(--border)', padding:'1rem 1.25rem' }}>
                  <div style={{ marginBottom:'1rem' }}>
                    <div style={{ ...labelStyle, marginBottom:8 }}>{t('export.sort_by')}</div>
                    <div style={{ display:'flex', gap:'0.5rem', flexWrap:'wrap' }}>
                      {[
                        { key:'number',    label: t('export.sort_number') },
                        { key:'jump_date', label: t('export.sort_date') },
                        { key:'altitude',  label: t('export.sort_altitude') },
                        { key:'delay',     label: t('export.sort_delay') },
                      ].map(({ key, label }) => (
                        <button key={key} onClick={() => toggleSort(key)} style={{ padding:'0.35rem 0.85rem', background: sortBy === key ? 'rgba(108,99,255,0.2)' : 'var(--bg3)', border: `1px solid ${sortBy === key ? 'var(--accent)' : 'var(--border)'}`, borderRadius: 8, color: sortBy === key ? 'var(--accent2)' : 'var(--muted)', fontFamily:'var(--font)', fontSize:'0.82rem', cursor:'pointer' }}>
                          {label}{sortIcon(key)}
                        </button>
                      ))}
                    </div>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(160px, 1fr))', gap:'0.75rem' }}>
                    <div><label style={labelStyle}>{t('export.date_from')}</label><input type="text" style={inputStyle} value={filterDateFrom} onChange={e => setFilterDateFrom(e.target.value)} placeholder="2024-01-01" maxLength={10} /></div>
                    <div><label style={labelStyle}>{t('export.date_to')}</label><input type="text" style={inputStyle} value={filterDateTo} onChange={e => setFilterDateTo(e.target.value)} placeholder="2024-12-31" maxLength={10} /></div>
                    <div><label style={labelStyle}>{t('export.city')}</label><select style={inputStyle} value={filterCity} onChange={e => setFilterCity(e.target.value)}><option value="">{t('export.all')}</option>{uniqueCities.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                    <div><label style={labelStyle}>{t('export.parachute')}</label><select style={inputStyle} value={filterParachute} onChange={e => setFilterParachute(e.target.value)}><option value="">{t('export.all')}</option>{uniqueParachutes.map(p => <option key={p} value={p}>{p}</option>)}</select></div>
                    <div><label style={labelStyle}>{t('export.aircraft')}</label><select style={inputStyle} value={filterAircraft} onChange={e => setFilterAircraft(e.target.value)}><option value="">{t('export.all')}</option>{uniqueAircrafts.map(a => <option key={a} value={a}>{a}</option>)}</select></div>
                    <div><label style={labelStyle}>{t('export.jump_type')}</label><select style={inputStyle} value={filterType} onChange={e => setFilterType(e.target.value)}><option value="">{t('export.all')}</option>{uniqueTypes.map(tp => <option key={tp} value={tp}>{tp}</option>)}</select></div>
                  </div>
                  {activeFiltersCount > 0 && (
                    <button onClick={resetFilters} style={{ marginTop:'0.85rem', background:'transparent', border:'1px solid var(--border)', borderRadius:8, color:'var(--muted)', padding:'0.35rem 0.85rem', fontFamily:'var(--font)', fontSize:'0.82rem', cursor:'pointer' }}>
                      ✕ {t('export.reset_filters')}
                    </button>
                  )}
                </div>
              )}
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'0.75rem', background:'var(--bg2)', border:'1px solid var(--border2)', borderRadius:'var(--r2)', padding:'1rem 1.25rem', marginBottom:'1rem' }}>
              <div style={{ display:'flex', alignItems:'center', gap:'1rem' }}>
                <label style={{ display:'flex', alignItems:'center', gap:'0.5rem', cursor:'pointer', fontSize:'0.9rem', fontWeight:500 }}>
                  <input type="checkbox" checked={filteredJumps.length > 0 && filteredJumps.every(j => selected.has(j.id))} onChange={toggleAll} style={{ width:16, height:16, accentColor:'var(--accent)', cursor:'pointer' }} />
                  {t('export.select_all')}
                </label>
                <span style={{ fontSize:'0.82rem', color:'var(--muted)', fontFamily:'var(--mono)' }}>
                  {selectedJumps.length} / {filteredJumps.length} {t('export.jumps_count')}
                ...
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'0.82rem' }}>
                  <thead>
                    <tr style={{ background:'var(--bg3)', borderBottom:'1px solid var(--border)' }}>
                      ...
                      <th style={thStyle(null)}>{t('export.col_lp')}</th>
                      <th style={thStyle('number')} onClick={() => toggleSort('number')}>{t('export.col_number')}{sortIcon('number')}</th>
                      <th style={thStyle('jump_date')} onClick={() => toggleSort('jump_date')}>{t('export.col_date')}{sortIcon('jump_date')}</th>
                      <th style={thStyle(null)}>{t('export.col_city')}</th>
                      <th style={thStyle(null)}>{t('export.col_parachute')}</th>
                      <th style={thStyle('altitude')} onClick={() => toggleSort('altitude')}>{t('export.col_altitude')}{sortIcon('altitude')}</th>
                      <th style={thStyle('delay')} onClick={() => toggleSort('delay')}>{t('export.col_delay')}{sortIcon('delay')}</th>
                      <th style={thStyle(null)}>{t('export.col_aircraft')}</th>
                      <th style={thStyle(null)}>{t('export.col_type')}</th>
                      <th style={thStyle(null)}>{t('export.col_notes')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {filteredJumps.length === 0 ? (
                      <tr><td colSpan={11} style={{ textAlign:'center', padding:'2rem', color:'var(--muted)' }}>{t('export.no_jumps')}</td></tr>
                    ) : filteredJumps.map((j, index) => (
                      <tr key={j.id} onClick={() => toggle(j.id)} style={{ borderBottom:'1px solid var(--border)', cursor:'pointer', background: selected.has(j.id) ? 'rgba(108,99,255,0.07)' : 'transparent', transition:'background 0.15s' }} onMouseEnter={e => { if (!selected.has(j.id)) e.currentTarget.style.background = 'rgba(255,255,255,0.02)' }} onMouseLeave={e => { e.currentTarget.style.background = selected.has(j.id) ? 'rgba(108,99,255,0.07)' : 'transparent' }}>
                        <td style={{ padding:'0.55rem 0.75rem', textAlign:'center' }}><input type="checkbox" checked={selected.has(j.id)} onChange={() => toggle(j.id)} onClick={e => e.stopPropagation()} style={{ width:15, height:15, accentColor:'var(--accent)', cursor:'pointer' }} /></td>
