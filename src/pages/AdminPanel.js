            ...
            <div key={u.id} style={{ background:'var(--bg2)', border:'1px solid var(--border)', borderRadius:'var(--r2)', overflow:'hidden' }}>
              ...
              <div style={{ display:'flex', alignItems:'center', gap:'0.75rem', padding:'0.85rem 1rem' }}>
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:'0.3rem', marginTop:'0.35rem' }}>
                    {u.is_premium && (
                      <span style={{ display:'inline-flex', alignItems:'center', gap:'0.25rem', background:'rgba(108,99,255,0.15)', border:'1px solid rgba(108,99,255,0.4)', borderRadius:20, padding:'0.1rem 0.55rem', fontSize:'0.68rem', color:'var(--accent2)', fontWeight:600 }}>
                        ⭐ Premium
                      </span>
                    )}
                    {!u.is_premium && activePerm.length === 0 && !u.is_admin && (
                      <span style={{ display:'inline-flex', alignItems:'center', gap:'0.25rem', background:'var(--bg3)', border:'1px solid var(--border)', borderRadius:20, padding:'0.1rem 0.55rem', fontSize:'0.68rem', color:'var(--muted)' }}>
                        brak uprawnień
                      </span>
                    )}
              ...
              {expanded === u.id && !u.is_admin && (
                <div style={{ borderTop:'1px solid var(--border)', padding:'1rem', background:'var(--bg3)' }}>
                  <div style={{ fontSize:'0.72rem', color:'var(--muted)', fontFamily:'var(--mono)', textTransform:'uppercase', letterSpacing:1, marginBottom:'0.75rem' }}>
