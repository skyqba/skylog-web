export default function JumpCard({ jump, onDelete }) {
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid var(--border2)', borderRadius: 'var(--r2)', padding: '1rem 1.1rem', marginBottom: '0.7rem', transition: 'border 0.2s' }}
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          {jump.jump_type && (
            <span style={{ background: 'rgba(52,211,153,0.12)', border: '1px solid rgba(52,211,153,0.3)', borderRadius: 6, padding: '0.2rem 0.6rem', fontFamily: 'var(--mono)', fontSize: '0.7rem', color: '#34D399', fontWeight: 500 }}>
              {jump.jump_type}
            </span>
          )}
          {jump.parachute && (
            <span style={{ background: 'rgba(108,99,255,0.15)', border: '1px solid rgba(108,99,255,0.3)', borderRadius: 6, padding: '0.2rem 0.6rem', fontFamily: 'var(--mono)', fontSize: '0.7rem', color: 'var(--accent2)', fontWeight: 500 }}>
              {jump.parachute}
            </span>
          )}
          <button onClick={() => onDelete(jump.id)} style={{ background: 'transparent', border: 'none', color: 'var(--muted)', cursor: 'pointer', fontSize: '0.9rem', padding: '0.2rem 0.35rem', borderRadius: 5, transition: 'color 0.2s' }}
            onMouseEnter={e => e.target.style.color = 'var(--danger)'}
            onMouseLeave={e => e.target.style.color = 'var(--muted)'}
            title="Usuń skok">✕</button>
        </div>
      </div>
